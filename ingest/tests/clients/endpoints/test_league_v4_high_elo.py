...
def test_get_grandmaster_league():
    ...
    get_grandmaster_league(
        client=mock_client, region=Region.EUW, queue=QueueType.RANKED_SOLO_5x5
    )

    mock_client.get_json.assert_called_once()
    assert (
        "grandmasterleagues/by-queue/RANKED_SOLO_5x5"
        in mock_client.get_json.call_args[1]["url"]
    )
...
def test_get_master_league():
    ...
    get_master_league(
        client=mock_client, region=Region.NA, queue=QueueType.RANKED_SOLO_5x5
    )
