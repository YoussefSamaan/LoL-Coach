...
class TestComputeFunctions:
    ...
    def test_compute_counter_min_samples_filter(self):
        ...
        assert "Ahri" not in ctr or "Zed" not in ctr["Ahri"]


class TestBuildTables:
    """Tests for main build_tables function."""

    @patch("ml.features.build_features.settings")
    @patch("ml.features.build_features.save_artifact_bundle")
    ...
    @patch("ml.features.build_features.settings")
    def test_build_tables_registers_latest_run(self, mock_settings, tmp_path: Path):
        mock_settings.data_root = tmp_path
        mock_settings.ingest.paths.parsed_dir = "parsed"
        mock_settings.artifacts_path = tmp_path / "artifacts"
        parsed_dir = tmp_path / "parsed"
        ...
        run_dir = build_tables(SmoothingConfig(min_samples=1))

        assert run_dir is not None
        registry = ModelRegistry(artifacts_root=mock_settings.artifacts_path)
        current = registry.get_current_version()
        assert current is not None
        assert current.run_id == run_dir.name

    @patch("ml.features.build_features.settings")
    def test_build_tables_generates_unique_run_ids(
        self, mock_settings, tmp_path: Path, monkeypatch
    ):
        ...
        class MockDatetime:
            _values = [
                ...
                datetime(2026, 1, 1, 0, 0, 0, 200000),
            ]
        ...
        monkeypatch.setattr("ml.features.build_features.datetime", MockDatetime)

        first_run = build_tables(SmoothingConfig(min_samples=1))
        second_run = build_tables(SmoothingConfig(min_samples=1))
    ...
    @patch("ml.features.build_features.settings")
    @patch("ml.features.build_features.save_artifact_bundle")
    def test_build_tables_malformed_data(
    ...
    @patch("ml.features.build_features.settings")
    @patch("ml.features.build_features.ArtifactStats")
    ...
    @patch("ml.features.build_features.settings")
    def test_build_tables_no_participants_extracted(
    ...
    @patch("ml.features.build_features.settings")
    @patch("ml.features.build_features.pd.DataFrame")
