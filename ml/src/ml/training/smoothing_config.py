...
class SmoothingConfig(BaseModel):
    ...
    role_alpha: float = Field(
        default=5.0, gt=0.0, description="Alpha for role-specific winrates"
    )
    role_beta: float = Field(
        default=5.0, gt=0.0, description="Beta for role-specific winrates"
    )
    pair_alpha: float = Field(
        default=10.0, gt=0.0, description="Alpha for pairwise synergy/counter"
    )
    pair_beta: float = Field(
        default=10.0, gt=0.0, description="Beta for pairwise synergy/counter"
    )
    min_samples: int = Field(
        default=5, ge=1, description="Minimum games for pair inclusion"
    )
