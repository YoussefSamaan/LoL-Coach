...
def compute_role_strength(
    df: pd.DataFrame, config: SmoothingConfig
...
def build_tables(config: SmoothingConfig | None = None) -> Path | None:
    ...
    # Include microseconds so back-to-back manual rebuilds produce distinct runs.
    ...
    registry.register(
        ...
        metrics={
            "rows": len(df),
            "champions": len(global_winrates),
            "synergy_pairs": synergy_pairs,
            "counter_pairs": counter_pairs,
        },
    )
    logger.info(f"Registered artifact run {run_id} as {version}")
