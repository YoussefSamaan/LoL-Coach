...
from ml.scoring.config import ScoringConfig


def _get_nested_any(
    mapping: ABCMapping[str, Any], *keys: str, default: Any = None
) -> Any:
...
def _get_nested_float(
    mapping: ABCMapping[str, Any], *keys: str, default: float = 0.0
) -> float:
...
def score_candidate(
    *,
    ...
    role_winrate = _get_nested_float(
        stats.get("role_strength", {}), role, candidate, default=0.5
    )
    ...
    synergy_lifts = []
    for ally in allies:
        lift_data = _get_nested_any(
            stats.get("synergy", {}), candidate, ally, default=0.0
        )
    ...
    for enemy in enemies:
        lift_data = _get_nested_any(
            stats.get("counter", {}), candidate, enemy, default=0.0
        )
        if isinstance(lift_data, dict):
