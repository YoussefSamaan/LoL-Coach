...
from core.domain.enums import Region, Role
...
class RecommendDraftRequest(BaseModel):
    ...
    allies: list[str] = Field(
        ...
        description="List of champion names already picked by your team",
    )
    enemies: list[str] = Field(
        default_factory=list,
        description="List of champion names picked by the enemy team",
    )
    bans: list[str] = Field(
        default_factory=list, description="List of banned champion names"
    )
